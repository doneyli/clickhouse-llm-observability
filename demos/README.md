...
|------|---------------|-----------------|-----|
| **[text-to-sql](text-to-sql/)** | Prompt chaining **with gate checks** (retry / abort / escalate) — NL → SQL over ClickHouse via MCP (LangChain); optionally a generate→critique→refine loop grounded in real EXPLAIN/execution evidence | Trace · Evaluate · Experiment · Deploy | `docker compose --profile demo run --rm text-to-sql python main.py` (gates only) · `… python main.py --refine` (Pattern #5) · client script: [`DEMO_SCRIPT.md`](text-to-sql/DEMO_SCRIPT.md) |
