...
| 5 | **Evaluate** | Decide if it's good enough to ship | Deterministic **code** evals, **LLM-as-a-Judge** (managed + custom), **two human annotation queues** (one of traces, one of whole **sessions**), and **conversation-level** scoring: a `conversation-snapshot` observation judged once per conversation, plus `create_score(session_id=…)` for the whole session | `agent/scoring.py`, `agent/conversation_scoring.py`, `evaluators/`, `scripts/seed_annotation_queue.py` |
