...
Python SDK — which this demo now uses (`langfuse>=4.10,<5.0`), so the action is
unblocked and both `RunnerContext` and `RegressionError` are importable from
`langfuse`. `prompt_gate.py` is still the gate on purpose: the threshold comparison
and the non-zero exit that blocks the deploy are the parts worth *showing* in a
demo, and the wrapper hides both. The threshold logic in `evaluate_gate()` is the
part to lift across when switching to the action.
