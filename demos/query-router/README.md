...
| Distributed trace nesting | ONE trace holds the router decision AND the chosen handler's full existing subtree, joined cross-process via `trace_context`. Note the trace is **named after the handler** (`text-to-sql-handler`, `vector-rag-handler`, `agentic-rag-handler`), not `route-and-dispatch`: the handler flushes per request while the router flushes at exit, so the handler's observation is ingested first and claims the trace name. Filtering on `name=route-and-dispatch` therefore shows only the **fallback** (in-process) traces — to see a dispatched one, look for a `*-handler` trace and confirm it contains `route-query` + `dispatch-*` alongside the handler's subtree. Verified: 28 observations for text-to-sql, 8 for vector-rag, 31–38 for agentic-rag. |
