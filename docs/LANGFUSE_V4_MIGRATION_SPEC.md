...
## 14. Cloud-side completion — read APIs + the dead judges (2026-08-13/14)

§11g left one item explicitly open: *"Cloud-side legacy rows not enumerated (no DB access;
the unstable evaluation-rules API would be needed)."* That is now done, against the Cloud
`real-estate` project (`cmrtgeanp0303ad0d6bjhud35`, `us.cloud.langfuse.com`).

**Cloud has ZERO legacy evaluator rows.** `GET /api/public/unstable/evaluation-rules`
returned `totalItems: 2`, both `target=observation`. This is real evidence rather than an
absence of evidence: the endpoint's response schema is a `oneOf` that explicitly includes
*"read-only legacy trace and dataset rules"*, so legacy rows would have appeared. The
self-hosted caveat in §13 stands unchanged — that is a different project on a different
server.

**But both Cloud rules were scoring nothing.** `Relevance` and `Helpfulness` were `enabled`,
`sampling: 1`, and had produced no scores for weeks: their filters still matched
`traceName ∈ {property-concierge, conversation}` and observation `name ∈ {property-concierge,
turn-1…turn-8}`, names the agent stopped emitting when the trace was renamed to
`handle-concierge-chat-message`. Repointed (with approval) to
`traceName=handle-concierge-chat-message` + `isRootObservation=true`, mappings untouched;
both now emit `source=EVAL` on the root observation. `seed_managed_evaluators.sh` emits that
same filter so the repo and the project cannot drift apart again — and it now matches the
root by **flag, not name**, because name-matching is precisely what failed silently here.
Note its idempotency check skips a rule whose filter has drifted; it will not repair one.

**Read APIs migrated off the deprecated endpoints** (4 call sites in `demos/real-estate`,
behind shared helpers in `agent/config.py`). Three findings that only testing surfaces:

| Finding | Consequence |
|---|---|
| `v2/observations` **rejects `parse_io_as_json`** outright (400); `input`/`output` are always raw strings | a naive port keeps "working" but compares strings to dicts |
| `fields=core,io` silently **omits `isRootObservation`** (it lives in `basic`) | root lookup returns `None`, so nothing matches |
| `v3/scores` **moved the score's target into `subject{kind,id,traceId}`** — the flat `observationId` key is gone | `score["observationId"]` reads `None` on every score; code concludes nothing is observation-scoped |

**Server split, measured — not read off the matrix.** Against self-hosted 3.221.1 with its
own keys: `GET /api/public/v2/observations` → **404** *"The observations v2 API is only
available in a Langfuse v4 write mode"*, while `GET /api/public/v3/scores` → **200**. So the
v2-vs-v3 boundary is per-endpoint. This is the same constraint that makes
`api.legacy.observations_v1` correct in `scripts/import-external-traces.py` (§Phase 3), and
it is why the `demos/real-estate` read helpers are Cloud-only: a `/v2/` 404 now reports the
server version rather than failing cryptically, since `LANGFUSE_HOST` defaults to
`localhost:3001`.
