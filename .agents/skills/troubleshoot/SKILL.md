...
4. **Match against known failures** — check the table in
   ...
   | Multi-turn agentic-rag chat in LibreChat doesn't group into one Langfuse Session (each turn a fresh random session) | `librechat.yaml`'s `rag-retriever` block lost its `headers.X-LibreChat-Conversation-Id: "{{LIBRECHAT_BODY_CONVERSATIONID}}"`, or `mcp-rag-retriever` wasn't rebuilt after a `server.py`/`mcp` bump. The conversation-id header read falls back **silently** to a random session (never errors). Restore the header, `docker compose --profile demo up -d --build mcp-rag-retriever`, start a **new** conversation. |
