...
- **Langfuse SDK**: **v4** (`langfuse>=4.7,<5.0`; `demos/real-estate` floors at `>=4.13.1` for multi-modal dataset items + `api.experiments.*`) — `start_as_current_observation(as_type=...)`, `propagate_attributes(...)`, `dataset.run_experiment(...)`. Put overall input/output on the **root observation** (`root.update(input=…, output=…)`): v4 is observations-first and derives a trace's input/output from it. Do **not** add `set_current_trace_io()` — it is deprecated and exists only for legacy *trace*-target LLM-as-a-judge rules. See `demos/text-to-sql/langfuse_config.py` for setup and [docs/LANGFUSE_V4_MIGRATION_SPEC.md](docs/LANGFUSE_V4_MIGRATION_SPEC.md) for the migration record. The self-hosted server stays on **v3** (`langfuse/langfuse:3.221.1`); the v4 SDK supports it (minimum server 3.63.0). Read-API caveat, measured against 3.221.1 — the boundary is **per-endpoint**, not "v2 vs v3": `GET /api/public/v2/observations`, `/v2/metrics`, `/experiments` and `/experiment-items` all **404** on a v3 server (so self-hosted code must use `api.legacy.observations_v1` / `api.legacy.metrics_v1` and the `datasets/{name}/runs` reads), while `/v3/scores`, `/v2/datasets` and `/otel/v1/traces` all work on v3. Cloud code has no such limit and must not use the deprecated endpoints — they are removed 2026-11-16; see §15 of [docs/LANGFUSE_V4_MIGRATION_SPEC.md](docs/LANGFUSE_V4_MIGRATION_SPEC.md) for what is migrated vs. blocked, and `demos/real-estate/agent/config.py` for the v4 read helpers.
